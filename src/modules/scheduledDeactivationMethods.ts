...
const sendDismissalEmails = async ({
    ...
    const subject =
        request.type === 'retirement'
            ? `${tr('Retirement')} ${request.user?.name} ${mainOrgUnit.name} (${phone})`
            : `${tr('Transfer from')} ${mainOrgUnit} ${tr('to')} ${
                  request?.newOrganizationUnit && request.newOrganizationUnit.name
              }(${request?.newOrganizationalGroup || ''}) ${request.user?.name} (${phone})`;

    const transferFrom = `${mainOrgUnit} > ${groups}`;
...
export const scheduledDeactivationMethods = {
    create: async (data: CreateScheduledDeactivation, sessionUserId: string) => {
        ...
        const sigmaMail = findSigmaMail(mails);

        const corporateMail = findCorporateMail(mails);
    ...
    cancel: async (data: CancelScheduledDeactivation, sessionUserId: string) => {
        ...
        const sigmaMail = findSigmaMail(emails);

        const corporateMail = findCorporateMail(emails);
