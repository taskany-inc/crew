...
export const getServerSideProps = createGetServerSideProps({
    ...
    action: async ({ stringIds, session, ssg }) => {
        ...
        const request = await ssg.userCreationRequest.getTransferInsideById.fetch(stringIds.requestId);
...
export default function EditTransferIncideRequest({ userId, requestId }: { userId: string; requestId: string }) {
    ...
    const { data: requestForm } = trpc.userCreationRequest.getTransferInsideById.useQuery(requestId);
