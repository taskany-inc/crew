            ...
            try {
                ...
                await historyEventMethods.create({ token: ctx.apiTokenId }, 'createUserCreationRequest', {
                    ...
                    after: {
                        ...
                        coordinatorIds: request.coordinators?.length
                            ? request.coordinators.map((c) => c.id).join(', ')
                            : undefined,
                        coordinatorLogins: input.coordinators?.length
                            ? input.coordinators.map((c: { login: string }) => c.login).join(', ')
                            : undefined,
                        lineManagerIds: request.lineManagers?.length
                            ? request.lineManagers.map((lm) => lm.id).join(', ')
                            : undefined,
                        lineManagerLogins: input.lineManagers?.length
                            ? input.lineManagers.map((lm: { login: string }) => lm.login).join(', ')
                            : undefined,
                        ...
                        buddyLogin: request.buddyLogin || undefined,
                        ...
                        recruiterLogin: request.recruiterLogin || undefined,
                    },
                });
